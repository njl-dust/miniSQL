...
class CatalogMeta {
 ...
 public:
  ...
  static CatalogMeta *NewInstance(MemHeap *heap) { return ALLOC_P(heap, CatalogMeta)(); }
...
class CatalogManager {
 public:
  ...
  ~CatalogManager();
	...
	{
		vector<string> names;
		for(auto &ele : table_names_)
			names.push_back(ele.first);

		return names;
	}

  dberr_t CreateTable(const std::string &table_name, TableSchema *schema, Transaction *txn, TableInfo *&table_info, const vector<string>& primaryKeys, const vector<string>& uniqueKeys);
