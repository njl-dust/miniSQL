...
IndexMetadata *IndexMetadata::Create(const index_id_t index_id, const string &index_name, const table_id_t table_id,
  ...
  return ALLOC_P(heap, IndexMetadata)(index_id, index_name, table_id, key_map);
}
...
uint32_t IndexMetadata::GetSerializedSize() const {
  uint32_t ofs = 4 + 4 + 4 + index_name_.length() + 4 + 4 + 4 * key_map_.size();
	return ofs;
}
  ...
  if (index_type == "bptree") {
    if (max_size <= 8)
      ...
      return nullptr;
		...
		cout << "unknown index type!" << endl;
    return nullptr;
  }
  return ALLOC_P(i_heap_, BPlusTreeIndex)(meta_data_->index_id_, key_schema_, max_size, buffer_pool_manager);
}
